...
generated quantities{
  ...
  for ( i in 2:21 )    // We only run from 2 to 21 to do a fair test with the AR(1) model, which uses lags so misses the first measurement and has N-1=20 measurements in total
    log_lik[i] = lognormal_lpdf( pelts[i,1] | log(pelts_pred[i,1]) , sigma[1] ) + lognormal_lpdf( pelts[i,2] | log(pelts_pred[i,2]) , sigma[2] );
}
